...
class WapController extends AddonsController {
	...
	function _initialize() {
		...
		$cart = D ( 'Cart' )->getMyCart ( $this->mid, true );
		if($cart != false){
	...
	function index() {
		...
		//公告
		$is_show['is_show'] = 1 ;
		$is_show['shop_id'] = $this->shop_id ;
		$notice = M("Shop_notice")->where($is_show)->find();
...
//		dump($goods);
		...
		$star = M("Shop_evaluate")->field('star')->where($where)->avg('star');
		$people = M("Shop_evaluate")->field('uid')->where($where)->count('uid');
//		dump($people);
//		lastsql();
	...
	function user_center() {
		//$follow_id = $this->mid;
		//$follow = get_followinfo ( $follow_id );//不明bug
		$uid['uid'] = $this->mid;
		$follow = M("User")->where($uid)->find();
		...
		// 我的评价
		$evaluateUrl = addons_url ( 'Shop://Wap/evaluate', array (
				'shop_id' => $this->shop_id
		) );
		$this->assign ( 'evaluateUrl', $evaluateUrl );
		$this->display ();
	}
	// 全部订单
	function myOrder() {
...
//		lastsql();
	...
	function close_order(){
		...
		$map ['status_code'] = 10;
		$unPayOrders = D ( 'Addons://Shop/Order' )->getOrderList ( $map );
		$this->assign ( 'close_orderClass', 'current' );
		$this->assign ( 'orderList', $unPayOrders );
		$this->display ( 'order_list' );
	}
	...
	function refund(){
		...
		$unPayOrders = D ( 'Addons://Shop/Order' )->getOrderList ( $map );
		// dump('--退款申请--');
		$this->assign ( 'refundClass', 'current' );
		$this->assign ( 'orderList', $unPayOrders );
		$this->display ( 'order_list' );
	}
	function refund_do(){
		if(IS_POST){
			...
			$data['send_code'] = I("send_code");
			$data['send_number'] = I("send_number");
			$data['express_number'] = I("express_number");
			$data['refund_reason'] = I("refund_reason");
			$data['order_id'] = I("order_id");
			$data['time'] = time();
			$data['uid'] = $this->mid;
			$rs = M("Shop_refund")->add($data);
			if($rs){
				$rs2 = D ( 'Addons://Shop/Order' )->setStatusCode(I("id"),8);
 				//$rs2 = M("Shop_order")->where($map)->save($data2);
				if($rs2){
					$this->success("申请成功",addons_url('Shop://Wap/refund'),1);
				}else{
					$this->error('申请失败');
				}
			}else{
...
/*		if ($orderInfo ['status_code'] == 3) { // 在配送中的订单自动从接口获取快递信息
	...
	function myComment(){
		$data['order_number'] = I("order_number");
		$data['star'] = I("star");
		$data['ev_content'] = I("ev_content");
		$data['goods_id'] = I("goods_id");
		$data['time'] = time();
		$data['uid'] = $this->uid;
		$data['shop_id'] = $this->shop_id;
		$rs = M("Shop_evaluate")->add($data);
		if($rs){
			$data2['order_number'] = I("order_number");
			$status['status_code'] = 7 ;
			$rs2 = M("Shop_order")->where($data2)->save($status);
			if($rs2){
				$this->success("评论成功",addons_url('Shop://Wap/waitCommentOrder'),1);
			}else{
				$this->error('评论失败');
			}
		}else{
	...
	function add_address() {
		if (IS_POST) {
			...
			if ($data ['from'] == 0) {
				redirect ( U ( 'myAddress', array ('shop_id' => $this->shop_id) ) );
			} else {
				redirect ( U ( 'choose_address', array ('shop_id' => $this->shop_id) ) );
			}
		}
	...
	function evaluate(){
		$where['uid'] = $this->uid;
//		dump(session());exit();
		$where['shop_id'] = session('wap_shop_id');
	...
	function user_do(){
		$uid['uid'] = I("uid");
		$where['truename'] = I("truename");
		$where['mobile'] = I("mobile");
		$where['weixin'] = I("weixin");
		//dump($where);
		$rs = M("User")->where($uid)->save($where);
		if ($rs) {
			$this->success (  '修改成功',addons_url('Shop://Wap/user_center',array('uid'=>$uid['uid'])),1 );
		} else {
			$this->success ( '修改失败' );
		}
	}
